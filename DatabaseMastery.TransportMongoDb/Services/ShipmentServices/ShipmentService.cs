...
namespace DatabaseMastery.TransportMongoDb.Services.ShipmentServices
{
    public class ShipmentService : IShipmentService
    {
        ...
        {
            var values = _mapper.Map<Shipment>(createShipmentDto);
            await _shipmentCollection.InsertOneAsync(values);
        }
        ...
        {
            var filter = Builders<Shipment>.Filter.Eq(x => x.CurrentStatus, "Delivered");
        ...
        {
            var filter = Builders<Shipment>.Filter.Eq(x => x.CurrentStatus, "Out for Delivery");
            return await _shipmentCollection.CountDocumentsAsync(filter);
        }

        public async Task<GetShipmentByIdDto> GetShipmentByIdAsync(string id)
        {
            var values = await _shipmentCollection.Find(x => x.ShipmentId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetShipmentByIdDto>(values);
        }
