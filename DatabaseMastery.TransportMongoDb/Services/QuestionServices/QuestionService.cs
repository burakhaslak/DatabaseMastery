...
namespace DatabaseMastery.TransportMongoDb.Services.QuestionServices
{
    public class QuestionService : IQuestionService
    {
        ...
        {
            var values = _mapper.Map<Question>(createQuestionDto);
            await _QuestionCollection.InsertOneAsync(values);
        }
        ...
        {
            var values = await _QuestionCollection.Find(x => x.QuestionId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetQuestionByIdDto>(values);
        }
