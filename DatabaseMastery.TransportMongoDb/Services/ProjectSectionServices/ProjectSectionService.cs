...
namespace DatabaseMastery.TransportMongoDb.Services.ProjectSectionServices
{
    public class ProjectSectionService : IProjectSectionService
    {
        ...
        {
            var values = _mapper.Map<ProjectSection>(createProjectSectionDto);
            await _ProjectSectionCollection.InsertOneAsync(values);
        }
        ...
        {
            var values = await _ProjectSectionCollection.Find(x => x.ProjectSectionId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetProjectSectionByIdDto>(values);
        }
