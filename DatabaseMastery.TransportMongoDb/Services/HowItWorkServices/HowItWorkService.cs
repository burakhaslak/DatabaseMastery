...
namespace DatabaseMastery.TransportMongoDb.Services.HowItWorkServices
{
    public class HowItWorkService : IHowItWorkService
    {
        ...
        {
            var values = _mapper.Map<HowItWork>(createHowItWorkDto);
            await _HowItWorkCollection.InsertOneAsync(values);
        }
        ...
        {
            var values = await _HowItWorkCollection.Find(x => x.HowItWorkId == id).FirstOrDefaultAsync();
