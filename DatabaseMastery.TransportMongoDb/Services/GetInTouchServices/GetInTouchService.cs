...
namespace DatabaseMastery.TransportMongoDb.Services.GetInTouchServices
{
    public class GetInTouchService : IGetInTouchService
    {
        ...
        {
            var values = _mapper.Map<GetInTouch>(createGetInTouchDto);
            await _GetInTouchCollection.InsertOneAsync(values);
        }
        ...
        {
            var values = await _GetInTouchCollection.Find(x => x.GetInTouchId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetGetInTouchByIdDto>(values);
        }
